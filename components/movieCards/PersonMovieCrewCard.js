...
export default function PersonMovieCrewCard({props, nav, darkMode}) {
  // console.log(props)
    ...
    <TouchableWithoutFeedback onPress={onPressHandler}>
      <View style={[darkMode?Style.darkCardContainer:Style.lightCardContainer,{ height: 68, width: 220, borderRadius: 4, flexDirection: "row" }]}>
        ...
        <View style={{ flex: 1, padding: 4 }}>
          <MonoTextBold numberOfLines={2} style={[darkMode?Style.mediumLightText:Style.mediumDarkText,{ marginBottom: 2 }]}>{props.title + getReleaseDate()}</MonoTextBold>
          <MonoTextBold numberOfLines={1} style={[darkMode?Style.smallLightText:Style.smallDarkText,{ opacity: 0.6, }]}>{props.job}</MonoTextBold>
        </View>
      </View>
