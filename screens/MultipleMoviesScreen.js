...
function MultipleMoviesScreen({ navigation, route, darkMode }) {
  ...
  const renderItem = ({item}) => {
    ...
    switch (item.media_type) {
      ...
      default:
        if(item.hasOwnProperty('title')) return <MultipleMovieCard props={item} darkMode={darkMode} nav={navigation} />;
        else return <MultipleTVCard props={item} darkMode={darkMode} nav={navigation} />;
    }
  }

  const pageNumberPressedHandler = () => {
  ...
  return (
    <View style={darkMode?Style.darkContainer:Style.lightContainer}>
      <FlatList
        ...
        numColumns={2}
        ListFooterComponent={<MultipleMoviesFooter pageNumberPressedHandler={pageNumberPressedHandler} page={movies.page} total_pages={movies.total_pages} total_results={movies.total_results} variables={route.params.variables} refetch={moviesResponse.refetch} darkMode={darkMode} />}
        data={movies.results}
